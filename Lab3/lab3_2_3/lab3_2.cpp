...
int main()
{
    ...
    cin >> k; // line
    cin >> l; // column
    unsigned num_line_el = k - 1, num_coolumn_el = l - 1; // переводим координаты элемента на язык массивов
    int** mat = new int*[m]; // создаём массив (матрицу) из m строк
    int max_the_max = 0;
    for (size_t j = 0; j < m; ++j)
        mat[j] = new int[n]; // в каждую из строк добавляем по n элементов
